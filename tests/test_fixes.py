...
def test_resumen_materialidad_no_name_error():
    ...
    """
    with (
        patch.object(materialidad_service, "sugerir_materialidad", return_value=MOCK_SUGERENCIA),
...
def test_sugerir_materialidad_uses_nested_perfil():
    ...
    with (
        patch.object(materialidad_service, "calcular_materialidad", return_value=mock_calculo),
...
def test_no_unused_import_in_obtener_materialidad_guardada():
    ...
    """
    source = pathlib.Path("domain/services/materialidad_service.py").read_text(encoding="utf-8")
