 ...
 * Detecta anomalias automaticamente en datos financieros del cliente
 */
...
interface Hallazgo {
  id: string
  descripcion: string
  nivel: 'CRITICO' | 'IMPORTANTE' | 'MENOR' | string
  norma: string
  riesgo: string
  auditoria: string[]
    ...
    try {
      ...
      const analysisData = await authFetchJson<ApiEnvelope<AnalysisResult>>(`/api/audit-analysis/${clienteId}/analyze`, {
        ...
        body: JSON.stringify({
          ...
          additional_data: {
            risk_signals: {
              areas: summarizeRiskSignals(riskAreas).summaries.slice(0, 5),
            },
          },
        }),
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Analizador Inteligente</h2>
          <p className="text-sm text-gray-600 mt-1">Detección automática de anomalías con IA</p>
        </div>
        <button
        ...
        <>
          ...
          {analysisIssue ? (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-6">
              <p className="text-amber-900 font-semibold">No se pudo completar el análisis inteligente</p>
              <p className="text-amber-800 mt-2">{analysisIssue}</p>
              <p className="text-amber-700 text-sm mt-3">Revisa la configuración de IA en el backend y vuelve a intentar.</p>
            </div>
          ) : result.hallazgos && result.hallazgos.length > 0 ? (
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Hallazgos Detectados ({result.hallazgos.length})</h3>

              <div className="space-y-3">
                {result.hallazgos.map((hallazgo, idx) => (
                  <button
                    key={`${hallazgo.id || idx}-${idx}`}
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              <p className="text-green-900">No se detectaron anomalías significativas en los datos</p>
            </div>
          )}
