...
def _register_editorial_fonts() -> dict[str, str]:
    ...
    for d in font_dirs:
        ...
        if inter_file is None:
            inter_file = next((p for p in [d / "Inter-Regular.ttf", d / "Inter.ttf"] if p.exists()), None)
        if news_file is None:
            news_file = next((p for p in [d / "Newsreader-Regular.ttf", d / "Newsreader.ttf"] if p.exists()), None)
...
def generate_executive_summary_pdf(
    ...
    *,
    ...
    risk = str(riesgo_global if riesgo_global is not None else dashboard_data.get("riesgo_global", "Medio")).strip().capitalize()
    mat = _safe_float(materialidad if materialidad is not None else dashboard_data.get("materialidad", 0.0), 0.0)
    period = str(periodo or dashboard_data.get("periodo", "Vigente"))
    sector = str(dashboard_data.get("sector", "N/A"))

    risk_color = ERROR_HEX if "alto" in risk.lower() else WARN_HEX if "medio" in risk.lower() else SUCCESS_HEX
...
def generar_pdf_resumen(
    ...
    riesgo_g = str(perfil.get("riesgo_global", {}).get("nivel", "N/A") if isinstance(perfil, dict) else "N/A").upper()
    ...
    nombre_c = str(datos_clave.get("nombre") or perfil.get("cliente", {}).get("nombre_legal", cliente))
    ...
    info = Table(
        [[
            ...
            Paragraph(f"Riesgo: {riesgo_g} | Materialidad: {_fmt_money(materialidad_plan)}", styles["body"]),
        ]],
        colWidths=[width * 0.22, width * 0.14, width * 0.15, width * 0.14, width * 0.16, width * 0.19],
    )
    info.setStyle(
        TableStyle(
            [
    ...
    # Resumen de riesgos
    story.append(Paragraph("Resumen de Riesgos", styles["section"]))
    riesgo_color_hex = ERROR_HEX if "ALTO" in riesgo_g else WARN_HEX if "MEDIO" in riesgo_g else SUCCESS_HEX
    riesgo_tbl = Table(
        [[
            Paragraph(f"<b>Nivel de Riesgo Global:</b> <font color='{riesgo_color_hex}'>{riesgo_g}</font>", styles["body"]),
            Paragraph(f"<b>Materialidad de Planeación:</b> {_fmt_money(materialidad_plan)}", styles["right"]),
        ]],
    ...
    story.append(HRFlowable(width=width, thickness=0.7, color=colors.Color(*_hex_to_rgb("#D8E2EF")), spaceAfter=8))
    ...
    if isinstance(ranking_areas, pd.DataFrame) and not ranking_areas.empty:
        ...
        for i, (_, row) in enumerate(df_r.head(12).iterrows(), start=1):
            ...
            rows.append([
                ...
                Paragraph(f"<b><font color='{score_hex}'>{score:.1f}</font></b>", styles["center"]),
                Paragraph(_fmt_money(abs(_safe_float(row.get("saldo_total", 0)))), styles["right"]),
            ])

        t = Table(rows, colWidths=[width * 0.06, width * 0.12, width * 0.40, width * 0.12, width * 0.30])
        t.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (-1, 0), colors.Color(*_hex_to_rgb(PRIMARY_HEX))),
                    ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
                    ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.Color(*_hex_to_rgb(SURFACE_LOW_HEX))]),
    ...
    if isinstance(variaciones, pd.DataFrame) and not variaciones.empty:
        ...
        c_nom = next((c for c in ["nombre", "nombre_cuenta", "descripcion"] if c in df_v.columns), None)
        c_imp = next((c for c in ["impacto", "variacion_absoluta", "abs_variacion_absoluta"] if c in df_v.columns), None)
        if c_imp:
            df_v["_abs"] = pd.to_numeric(df_v[c_imp], errors="coerce").abs().fillna(0)
            df_v = df_v.sort_values("_abs", ascending=False)

        rows = [[Paragraph("<b>Cuenta</b>", styles["body"]), Paragraph("<b>Impacto</b>", styles["right"])]]
        for _, row in df_v.head(8).iterrows():
            ...
            rows.append([
                ...
                Paragraph(f"<font color='{color}'>{_fmt_money(abs(impact))}</font>", styles["right"]),
            ])
    ...
    story.append(Paragraph(f"Generado por SocioAI - {datetime.now().strftime('%d/%m/%Y')} - Uso interno confidencial", styles["small"]))
