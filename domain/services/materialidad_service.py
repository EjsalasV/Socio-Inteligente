...
def guardar_sugerencia_materialidad(
    cliente: str, materialidad_elegida: Optional[float] = None
) -> bool:
    """
    ...
    try:
        ...
        if not (
            calculo["materialidad_minima"] <= materialidad_elegida <= calculo["materialidad_maxima"]
        ):
            print(
                f"⚠️  Advertencia: materialidad ${materialidad_elegida:,.0f} fuera del rango recomendado"
            )
