...
def render_calidad_tab(ws: dict[str, Any]) -> None:
    ...
    st.markdown(
        f"- Riesgo fraude en ingresos presente: {'Si' if rim.get('ingresos_presente') else 'No'}"
    )
    st.markdown(
        f"- Riesgo override gerencia presente: {'Si' if rim.get('gerencia_presente') else 'No'}"
    )
    st.markdown(
    ...
    req = (
        calidad.get("procedimientos_materialidad", {})
        if isinstance(calidad.get("procedimientos_materialidad", {}), dict)
        else {}
    )
    ...
    st.markdown(
        f"- Relacionada a fraude en ingresos: {'Si' if req.get('riesgo_fraude_relacionado') else 'No'}"
    )

    ctrl = (
        calidad.get("pruebas_control_walkthrough", {})
        if isinstance(calidad.get("pruebas_control_walkthrough", {}), dict)
        else {}
    )
    st.markdown("**Pruebas de control / walkthrough**")
    st.markdown(
        f"- Hay pruebas control/recorrido: {'Si' if ctrl.get('hay_control_o_walkthrough') else 'No'}"
    )
    st.markdown(
        f"- Soporte de base de muestra/transaccion: {'Si' if ctrl.get('tiene_soporte_base') else 'No'}"
    )

    ing = (
        calidad.get("ingresos_metodologia", {})
        if isinstance(calidad.get("ingresos_metodologia", {}), dict)
        else {}
    )
    st.markdown("**Metodologia de ingresos**")
    if ing.get("aplica"):
        st.markdown(
            f"- Marco aplicado: {normalize_text(ing.get('marco', 'no_disponible')) or 'no_disponible'}"
        )
        ...
        st.markdown(
            f"- Checklist: {', '.join([str(x) for x in checklist]) if checklist else 'No disponible'}"
        )
        st.markdown(
            f"- Faltantes: {', '.join([str(x) for x in faltantes]) if faltantes else 'Ninguno'}"
        )
    else:
        st.info("No aplica para esta area.")

    gas = (
        calidad.get("gastos_metodologia", {})
        if isinstance(calidad.get("gastos_metodologia", {}), dict)
        else {}
    )
    ...
    est = (
        calidad.get("estimaciones_nia540", {})
        if isinstance(calidad.get("estimaciones_nia540", {}), dict)
        else {}
    )
    st.markdown("**NIA 540 - estimaciones contables**")
    if est.get("aplica"):
        enfoques = (
            est.get("enfoques_detectados", [])
            if isinstance(est.get("enfoques_detectados", []), list)
            else []
        )
        st.markdown(
            f"- Enfoques detectados: {', '.join([str(x) for x in enfoques]) if enfoques else 'Ninguno'}"
        )
        sugerencias = (
            est.get("sugerencias", []) if isinstance(est.get("sugerencias", []), list) else []
        )
    ...
    hold = (
        calidad.get("holding_sensibilidad", {})
        if isinstance(calidad.get("holding_sensibilidad", {}), dict)
        else {}
    )
    if hold.get("aplica"):
        st.markdown("**Sensibilidad de calidad para holding**")
        obs = (
            hold.get("observaciones", []) if isinstance(hold.get("observaciones", []), list) else []
        )
        if obs:
            for o in obs:
                st.markdown(f"- {o}")
