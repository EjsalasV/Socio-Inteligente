...
Orquestador del flujo de auditoría:
cliente → perfil → TB → variaciones → ranking de áreas
"""
...
# Ensure Windows terminals with cp1252 can print unicode output safely.
...
def run_cliente(cliente: str) -> Optional[Dict[str, Any]]:
    print("\n" + "=" * 70)
    print(f"🚀 INICIANDO ANÁLISIS: {cliente}")
    print("=" * 70)
    ...
    if not perfil:
        print(f"❌ Cliente '{cliente}' no encontrado o perfil inválido")
        return {
            "status": "ERROR",
            "cliente": cliente,
    ...
    print(f"✅ Cliente cargado: {datos_clave.get('nombre')}")
    print(f"   RUC: {datos_clave.get('ruc')}")
    print(f"   Sector: {datos_clave.get('sector')}")
    ...
    if tb is None or tb.empty:
        print(f"❌ No se pudo cargar el trial balance")
        return {
            "status": "ERROR",
            "cliente": cliente,
    ...
    print(f"✅ TB cargado: {tb.shape[0]} cuentas")
    ...
    if resumen_var:
        print(f"✅ Variaciones calculadas: {resumen_var.get('total_cuentas_variacion', 0)} cuentas")
        print(f"   Mayor variación: ${resumen_var.get('mayor_variacion', 0):,.0f}")
        print(f"   Top cuentas: {', '.join(resumen_var.get('cuentas_top_5', [])[:2])}")
    else:
        print("ℹ️  Sin variaciones significativas")
        resumen_var = {
    ...
    if ranking is not None and not ranking.empty:
        print("✅ Ranking de áreas calculado:")
        ...
        print("\n   📊 Indicadores clave:")
        print(f"   - Áreas alto riesgo: {indicadores.get('areas_alto_riesgo', 0)}")
        print(f"   - Áreas medio riesgo: {indicadores.get('areas_medio_riesgo', 0)}")
        print(f"   - Áreas bajo riesgo: {indicadores.get('areas_bajo_riesgo', 0)}")
        print(f"   - Concentración principal área: {indicadores.get('concentracion_principal_area', 0):.1f}%")
    else:
        print("⚠️  No se pudo calcular ranking")
        ranking = None
    ...
    print(f"   Áreas alto riesgo: {resultado['areas_riesgo'].get('areas_alto_riesgo', 0)}")
